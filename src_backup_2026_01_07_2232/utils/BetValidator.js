...
// Numbers Arrays needed for simulation
    ...
    for (let num = 0; num <= 36; num++) {
        ...
        Object.entries(currentBets).forEach(([betId, amount]) => {
            if (doesNumberWin(num, betId)) {
                ...
                // Win = Bet returned + Profit
                // Usually limits apply to Net Profit (Payout), but sometimes Total Return.
                // User said "MAX_WIN" (Max Ganancia). Usually means Net Profit.
                // Payout formula in table: amount + (amount * mult)
                // Net Profit = amount * mult
                roundWinnings += (amount * mult)
            }
        })
