...
SYSTEMS.forEach(sysNum => {
    const sectors = getRelativeSectors(sysNum);
    ...
    RESULTS[sysNum] = {
        Nucleo: { cost: nucleoBets.length, nums: sectors.nucleo.length },
        Vecinos: { cost: vecinosBets.length, nums: sectors.vecinos.length },
        Huerfanos: { cost: orphelinsBets.length, nums: sectors.orphelins.length },
        Tiers: { cost: tiersBets.length, nums: sectors.tiers.length }
    };
});
